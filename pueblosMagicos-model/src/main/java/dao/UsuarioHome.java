...
public class UsuarioHome {
	...
	public Usuario findById(java.lang.Integer id) {
		...
		try {
			Usuario instance = (Usuario) sessionFactory.getCurrentSession().get("dao.Usuario", id);
			if (instance == null) {
	...
	public List<Usuario> findByExample(Usuario instance) {
		...
		try {
			List<Usuario> results = (List<Usuario>) sessionFactory.getCurrentSession().createCriteria("dao.Usuario")
